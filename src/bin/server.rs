...
fn handle_connection(stream: &mut TcpStream, capturer: &mut Capturer, (w, h): (u32, u32)) {
    ...
    loop {
        ...
        if waiting_package_direct {
            ...
            stream.write_all(&[0x33, 0x34, 0x35, 0x36]).unwrap();
            stream.flush().unwrap();
            // Send frame dimensions.
            stream.write_all(&w.to_le_bytes()).unwrap();
            stream.write_all(&h.to_le_bytes()).unwrap();
            stream.flush().unwrap();
            ...
            for Bgr8 { r, g, b, .. } in &ps {
                ...
                if chunks.0 == CHUNK_SIZE {
                    stream.write_all(chunks.1).unwrap();
                    stream.flush().unwrap();
                    chunks.0 = 0;
                    chunks.1.fill(0);
                }
    ...
    for stream in listener.incoming() {
        match stream {
            Ok(mut stream) => {
                ...
                handle_connection(&mut stream, &mut capturer, (w, h));
            }
            Err(e) => {
                println!("Error: {}", e);
            }
